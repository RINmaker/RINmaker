...
private:
    ...
    pair<double, double> getSigmaEpsilon(entities::atom const& donor, entities::atom const& acceptor)
    {
        function<bool(string const&, int, string const&, int)> compare = [&](string const& donor_element, int donor_charge, string const& acceptor_element, int acceptor_charge)
        {
        ...
        if (compare("N", 0, "N", 0)) return make_pair(1.99, -3.00);
        if (compare("N", 0, "O", 0)) return make_pair(1.89, -3.50);
        if (compare("O", 0, "N", 0)) return make_pair(1.89, -4.00);
        if (compare("O", 0, "O", 0)) return make_pair(1.79, -4.25);
        if (compare("N", 1, "N", 0)) return make_pair(1.99, -4.50);
        if (compare("N", 1, "O", 0)) return make_pair(1.89, -5.25);

        if (compare("N", 0, "O", -1)) return make_pair(1.89, -5.25);
        if (compare("N", 1, "O", -1)) return make_pair(1.89, -7.00);
        if (compare("O", 0, "O", -1)) return make_pair(1.79, -6.375);

        return make_pair(0, 0); //TODO log: non dovrebbe accadere
    }
